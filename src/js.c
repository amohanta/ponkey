...
#define ARENA_LOG "/tmp/arena_log"
#define ATOM_LOG "/tmp/atom_log"
#define GLOBAL_LOG "/tmp/global_log"
#define OBJECT_LOG "/tmp/object_log"

//P script to initialize document class
#define INITSCRIPT "initscript.js"
...
//P Initialization scripts to be inserted here
...
#define INITUSINGSCRIPT
//P I am sorry I'm doing this
#ifdef INITUSINGSCRIPT
	JSScript *initscript;
	FILE *initfile = fopen(INITSCRIPT , "r");
        if (!initfile) {
		fprintf(stderr, "No initialisation script found in current path.\n");
        }
 
	initscript = JS_CompileFileHandle(cx, obj, "initscript.js", initfile);
	if(initscript) {
		if(!compileOnly)
		(void)JS_ExecuteScript(cx, obj, initscript, &result);
//P consider result
	}
#endif
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (strcmp(bytes, "arena") == 0) {
           ...
           fclose(fOut);
            ...
            fOut = fopen(GLOBAL_LOG, "w");
            if(!fOut){
                fprintf(gErrFile, "unable to open GLOBAL_LOG\n");
                return JS_FALSE;
            }
            DumpScope(cx, cx->globalObject, fOut);
            fclose(fOut);
        } else {
            ...
            if (obj)
                fOut = fopen(OBJECT_LOG, "a");
                if(!fOut){
                        fprintf(gErrFile, "unable to open OBJECT_LOG file\n");
                        return JS_FALSE;
                }
                fprintf(fOut, "\nObject Stats for %s:\n", bytes);
                DumpScope(cx, obj, fOut);
                fclose(fOut);
        }
    }
    return JS_TRUE;
...
/* //P Major Change here this was defined so that the classes
 * //P are initialized later not on creation of js shell */
...
static JSBool
setTimeout(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
              FILE *fOut;

                    const char* bytes;
                          int milliseconds;
                                bytes = JS_GetStringBytes(js_ValueToString(cx, argv[0]));
                                      milliseconds = JSVAL_TO_INT(argv[1]);

                                            fOut = fopen("home/devil/Typ/logs/function.log", "a");
                                                  if (fOut == NULL)
                                                                   return JS_FALSE;

                                                        fprintf(fOut, "SETTIMEOUT: %s %d\n", bytes, milliseconds);
                                                              fclose (fOut);
                                                                    return JS_TRUE;
}
...
static JSBool
clearTimeout(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
              FILE *fOut;

              int timeID;
              timeID = JSVAL_TO_INT(argv[0]);

              fOut = fopen("/tmp/js_function.log", "a");
              if (fOut == NULL)
                return JS_FALSE;

              fprintf(fOut, "CLEARTIMEOUT: %d\n", timeID);
              fclose (fOut);
              return JS_TRUE;
}
...
main(int argc, char **argv, char **envp)
{
    ...
    * so we need to unbuffer then to get a reasonable prompt          */
    ...
    if (!glob)
        return 1;
//P testing creation of document
#undef LAZY_STANDARD_CLASSES
...
#endif
        if (!JS_DefineFunction(cx, glob, "setTimeout", setTimeout, 3, 0))
            return 1;
        if (!JS_DefineFunction(cx, glob, "clearTimeout", clearTimeout, 1, 0))
            return 1;
